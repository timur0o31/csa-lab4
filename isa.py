...
def write_hex_data(filename, data, start_addr=0):
    with open(filename, "w", encoding="utf-8") as f:
        for i, value in enumerate(data, start=start_addr):
            ...
            bin_str = f"{value & 0xFFFFFFFF:032b}"
            f.write(f"{i} - {hex_str} - {bin_str}\n")
...
def instruction_to_hex(instr) -> str:
    ...
    return f"{machine_code:08X}", machine_code
...
def write_hex_instructions(filename, instructions):
    with open(filename, "w", encoding="utf-8") as f:
        for instr in instructions:
            hexcode, bincod = instruction_to_hex(instr)
            bincode = f"{bincod:032b}"
            mnemonic = instruction_to_mnemonic(instr)
            address = instr['index']
            f.write(f"{address} - {hexcode} - {bincode} - {mnemonic}\n")
